...
function updateCartItem(obj, rowid,item_id){
  ...
  recalculateCartTotals();
    ...
    $.get("<?php echo base_url('admin/updateItemQty/'); ?>",{rowid:rowid, qty:normalizedQty,item_id:item_id}, function(resp){
        if(String(resp).trim() == 'ok'){
      obj.setAttribute('data-old-qty', normalizedQty);
      recalculateCartTotals();
        }else{
      obj.value = oldQty;
      obj.setAttribute('data-old-qty', oldQty);
      recalculateCartTotals();
      showQuantityWarning(maxStock > 0 ? 'Only ' + maxStock + ' item(s) available in stock.' : 'Selected quantity is not available.');
        }
    });
}
    ...
    <?php echo form_open("admin/sell"); ?>
     ...
     <!-- <select type="number" class="form-control" id='cartp' onchange="updateCartItem(this, '<?php echo $cart_id; ?>','<?php echo $item_id; ?>')" style="width: 80px">
       ...
       <input type="hidden" name="quantity[]" value="<?php echo $item["qty"]; ?>" class="evamo-qty-hidden">
            ...
            <input type="hidden" name="profit[]" value="<?php echo $item['subtotal'] - $item['buy_price'] * $item['qty']; ?>" class="evamo-profit-hidden">
